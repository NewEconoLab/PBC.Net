...
static void CRT(mpz_t x, mpz_ptr *v, mpz_ptr *m, int t) {
  mpz_t u;
  mpz_t *C = _alloca(t * sizeof(mpz_t));
  int i, j;
...
void index_calculus_step1(mpz_t *ind, int r, mpz_t g, mpz_t q,
  ...
  darray_t **matrix = _alloca(fac->count * sizeof(darray_t*));
  int *minfound = _alloca(fac->count * sizeof(int));
...
static void index_calculus_step2(mpz_t x, mpz_t *ind, int r,
  ...
  mpz_t s;
  mpz_t z, z1;
  mpz_t *rel = _alloca(r * sizeof(mpz_t));
  int i;
